...
(function (global, factory) {
  ...
  var setParameters = function (params) {
    ...
    var $closeButton = modal.querySelector('.' + swalClasses.close); // Title
    ...
    $cancelBtn.innerHTML = params.cancelButtonText; // Set buttons to selected background colors
    ...
    addClass($cancelBtn, params.cancelButtonClass); // Buttons styling
  ...
  var openModal = function (animation, onComplete) {
    ...
    show(modal); // scrolling is 'hidden' until animation is done, after that 'auto'
  ...
  function modalDependant() {
    ...
    setParameters(params); // Modal interactions
    ...
    return new Promise(function (resolve, reject) {
      ...
      var $cancelButton = getCancelButton(); // Reverse buttons if neede d
      ...
      window.onkeydown = handleKeyDown; // Loading state
      ...
      sweetAlert.hideLoading();
      sweetAlert.resetValidationError(); // inputs
      ...
      for (i = 0; i < inputTypes.length; i++) {
        ...
        input = getInput(inputTypes[i]); // set attributes
      ...
      openModal(params.animation, params.onOpen); // Focus the first element (input or button)

      setFocus(-1, 1); // fix scroll

      sweetContainer.scrollTop = 0; // Observe changes inside the modal and adjust height
  ...
  sweetAlert.close = sweetAlert.closeModal = function (onComplete) {
    ...
    addClass(modal, swalClasses.hide); // Reset icon animations
